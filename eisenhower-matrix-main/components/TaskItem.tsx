...
export function TaskItem({
  ...
  const body = (
    <div>
      ...
      {/* Căn cùng lề trái với ô tích phía trên, không thụt theo nội dung */}
      <div className="flex flex-wrap items-center gap-1 mt-2">
        {task.department && (
          <span
            className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[11px] font-semibold rounded-sm"
        ...
        {task.category && (
          <span
            className="inline-flex items-center px-1.5 py-0.5 text-[11px] font-semibold rounded-sm"
        ...
        <span
          className="inline-flex items-center px-1.5 py-0.5 text-[11px] font-bold rounded-sm"
          style={{ backgroundColor: statusColor, color: '#FFFFFF' }}
        >
          {t(`status.${task.status}`)}
        </span>
        {task.dueDate && (
          <Tooltip title={`${t('fields.dueDate')}: ${formatDate(task.dueDate)}`}>
            <span
              className="inline-flex items-center gap-1 px-1.5 py-0.5 text-[11px] font-bold rounded-sm"
